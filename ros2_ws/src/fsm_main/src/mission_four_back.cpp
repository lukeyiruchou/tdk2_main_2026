...
class MissionFourBackNode : public BaseStateNode {//here
public:
    // 呼叫父類建構子，定義節點名稱
    MissionFourBackNode() : BaseStateNode("mission_four_back_node") {}//here
...
protected:
    ...
    CallbackReturn on_configure(const rclcpp_lifecycle::State & state) override {
        ...
        RCLCPP_INFO(get_logger(), "正在配置4b：載入動作序列庫...");
    ...
    CallbackReturn on_activate(const rclcpp_lifecycle::State & state) override {
        ...
        RCLCPP_INFO(get_logger(), "mission 4b activate");
        progress_ = 0.0;

        move_along_path("PATH_4b_0");

        timer_ = this->create_wall_timer(
            std::chrono::seconds(1), 
            std::bind(&MissionFourBackNode::execute_step, this));//here
    ...
    CallbackReturn on_deactivate(const rclcpp_lifecycle::State & state) override {
        // 1. 停止此狀態的工作
        RCLCPP_INFO(get_logger(), "mission 4b stop");
        if (timer_) {
            timer_->cancel();
        }
    ...
    void execute_step() {
        ...
        if (is_path_arrived_) {
            ...
            // 階段 3：定點任務也功德圓滿，切換到下一關 Lifecycle 狀態
                ...
                RCLCPP_INFO(get_logger(), "✅ Mission 4b complete！");
                timer_->cancel();
...
int main(int argc, char ** argv) {
    ...
    auto node = std::make_shared<MissionFourBackNode>();//here
