...
class MissionTwoNode : public BaseStateNode {
public:
    // 呼叫父類建構子，定義節點名稱
    MissionTwoNode() : BaseStateNode("mission_two_node") {}
...
protected:
    ...
    CallbackReturn on_configure(const rclcpp_lifecycle::State & state) override {
        ...
        RCLCPP_INFO(get_logger(), "mission two configure");
    ...
    CallbackReturn on_activate(const rclcpp_lifecycle::State & state) override {
        ...
        RCLCPP_INFO(get_logger(), "mission two activate");
        progress_ = 0.0;

        move_along_path("PATH_2_0");

        timer_ = this->create_wall_timer(
            std::chrono::seconds(1), 
            std::bind(&MissionTwoNode::execute_step, this));
    ...
    CallbackReturn on_deactivate(const rclcpp_lifecycle::State & state) override {
        // 1. 停止此狀態的工作
        RCLCPP_INFO(get_logger(), "mission two stop");
        if (timer_) {
            timer_->cancel();
        }
...
int main(int argc, char ** argv) {
    ...
    auto node = std::make_shared<MissionTwoNode>();
