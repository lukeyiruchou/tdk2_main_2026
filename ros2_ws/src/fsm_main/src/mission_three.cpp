...
class MissionThreeNode : public BaseStateNode {//here
public:
    // 呼叫父類建構子，定義節點名稱
    MissionThreeNode() : BaseStateNode("mission_three_node") {}//here
...
protected:
    ...
    CallbackReturn on_configure(const rclcpp_lifecycle::State & state) override {
        ...
        RCLCPP_INFO(get_logger(), "正在配置3：載入動作序列庫...");
    ...
    CallbackReturn on_activate(const rclcpp_lifecycle::State & state) override {
        ...
        RCLCPP_INFO(get_logger(), "mission 3 activate");
        progress_ = 0.0;

        move_along_path("PATH_3_0");

        timer_ = this->create_wall_timer(
            std::chrono::seconds(1), 
            std::bind(&MissionThreeNode::execute_step, this));//here
    ...
    CallbackReturn on_deactivate(const rclcpp_lifecycle::State & state) override {
        // 1. 停止此狀態的工作
        RCLCPP_INFO(get_logger(), "mission 3 stop");
        if (timer_) {
            timer_->cancel();
        }
...
private:
    ...
    void execute_step() {
        ...
        if (is_path_arrived_) {
            if (progress_ < 0.1) {
                progress_ += 0.1; // 
                move_along_path("PATH_3_1");
                RCLCPP_INFO(get_logger(), "車體已到站！正在執行任務... 進度: %.2f", progress_);
            } 
            else if(progress_ < 0.2){
                progress_ += 0.1;
                move_along_path("PATH_3_2");
                RCLCPP_INFO(get_logger(), "車體已到站！正在執行任務... 進度: %.2f", progress_);
            }
            else if(progress_ < 0.3){
                progress_ += 0.1;
                move_along_path("PATH_3_3");
                RCLCPP_INFO(get_logger(), "車體已到站！正在執行任務... 進度: %.2f", progress_);
            }
            else if(progress_ < 0.4){
                progress_ += 0.1;
                move_along_path("PATH_3_4");
                RCLCPP_INFO(get_logger(), "車體已到站！正在執行任務... 進度: %.2f", progress_);
            }
            else if(progress_ < 0.5){
                progress_ += 0.1;
                move_along_path("PATH_3_5");
                RCLCPP_INFO(get_logger(), "車體已到站！正在執行任務... 進度: %.2f", progress_);
            }
            else if(progress_ < 0.6){
                progress_ += 0.1;
                move_along_path("PATH_3_6");
                RCLCPP_INFO(get_logger(), "車體已到站！正在執行任務... 進度: %.2f", progress_);
            }
            else if(progress_ < 0.7){
                progress_ += 0.1;
                move_along_path("PATH_3_7");
                RCLCPP_INFO(get_logger(), "車體已到站！正在執行任務... 進度: %.2f", progress_);
            }
            else if (progress_ < 1){
                progress_ += 0.1; // 
            ...
            // 階段 3：定點任務也功德圓滿，切換到下一關 Lifecycle 狀態
                ...
                RCLCPP_INFO(get_logger(), "✅ Mission three complete！移交大腦狀態機。");
                timer_->cancel();
...
int main(int argc, char ** argv) {
    ...
    auto node = std::make_shared<MissionThreeNode>();//here
